...
* `session-title`。
*/
async function callTitleModel(ctx, pluginName, settings, request, state) {
...
*/
function composeTitle(date, type, topic, format) {
	const head = `${date}${format.separator}${type}`;
	const clean = normalizeSessionTitle(topic, format.maxBytes);
	return truncateTitleUtf8(clean ? `${head}${format.separator}${clean}` : head, format.maxBytes);
}
/**
* 规则模式：类型与主题都取自首条消息。
*
...
var SessionTitlePatternProvider = class {
	...
	*/
	...
	async generate(request) {
		...
		if (this.config.mode !== "llm") return {
			title: buildRuleTitle(request.messages, this.config),
			messageSeqs
		};
		const state = this.stateOf(request.session.id);
		try {
			const { text, route } = await callTitleModel(this.ctx, name, this.config, request, state);
			...
			const title = composeTitle(formatPatternDate(/* @__PURE__ */ new Date()), type, parsed.topic, this.config);
...
*/
function trackRecomputes(ctx, config, states) {
...
function apply(ctx, config) {
	...
	ctx.effect(() => dispose);
	if (config.mode === "llm") {
		if (config.provider.length > 0 !== config.model.length > 0) logger.warn(`provider / model 必须成对配置，当前 provider=${JSON.stringify(config.provider)}、model=${JSON.stringify(config.model)}，将忽略这两项并跟随会话主模型。`);
