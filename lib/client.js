				...
				return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							className: "stp-hint",
							children: `已隐藏 ${hiddenCount} 条。悬停侧边栏的会话行可隐藏 / 取消隐藏，「工作区」那行右侧的眼睛是一键全显 / 全隐；隐藏只影响侧边栏显不显示，会话本身、搜索与标题都照常。`
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								...
								onClick: () => {
									...
									scope.unset("revealHiddenWorkspaces").catch(() => setFailed(true));
								},
								children: "全部取消隐藏"
							})
						})
...
`;
		...
		*/
		function workspaceKeyOfRow(element) {
			let fiber = fiberOf(element);
			for (let depth = 0; fiber !== void 0 && depth < MAX_FIBER_DEPTH; depth += 1) {
				const group = readRecord(readRecord(fiber.memoizedProps)?.group);
				if (group !== void 0) return readString(group.workspaceId) ?? UNGROUPED_KEY;
				fiber = fiber.return ?? void 0;
			}
		}
...
`;
		...
		*/
		function revealOf(workspaceKey, config) {
			const override = config.revealHiddenWorkspaces[workspaceKey];
			return override === void 0 ? config.revealHiddenAll : override;
		}
		...
		function readBooleanRecord(value) {
			...
			const result = {};
			for (const [key, entry] of Object.entries(value)) if (entry === true || entry === false) result[key] = entry;
			return result;
		}
		/**
		* 从设置快照里解析出隐藏配置。
		...
		*/
		...
		function shouldUnset(field, value) {
			...
			if (field === "revealHiddenWorkspaces") return value !== null && typeof value === "object" && Object.keys(value).length === 0;
			return false;
		}
		var Store = class {
			scope;
			...
			toggleAll() {
				this.write({
					revealHiddenAll: !this.local.revealHiddenAll,
					revealHiddenWorkspaces: {}
				});
			}
			...
			absorb() {
				...
				if (this.dirty.size === 0) this.local = incoming;
					...
					revealHiddenWorkspaces: this.dirty.has("revealHiddenWorkspaces") ? this.local.revealHiddenWorkspaces : incoming.revealHiddenWorkspaces
				};
				this.emit();
			}
			write(patch) {
		...
		*/
		function buildWorkspaceIndex(workspaces) {
			const index = /* @__PURE__ */ new Map();
			if (workspaces === void 0) return index;
			for (const item of workspaces.list.getSnapshot().items) for (const sessionId of item.sessionIds) index.set(sessionId, item.workspaceId);
			return index;
		}
		/** 行识别失效的告警只打一次，避免每帧刷屏。 */
		...
		function installHiddenSessions(ctx, scope) {
			...
			*/
			const applySessionRow = (row, config, hidden, current, stats) => {
				...
				setRowVisibility(row, rowMode(isHidden, isHidden && revealOf(workspaceKeyOf(id), config), id === current));
			...
			*/
			const applyProjectRow = (row, config, hiddenWorkspaces) => {
				const actions = row.querySelector(bySuffix("rowActions"));
				if (actions === null) return;
				const existing = eyeIn(actions, "workspace");
				const key = workspaceKeyOfRow(row);
				if (key === void 0 || key === "" || !hiddenWorkspaces.has(key)) {
					existing?.remove();
					return;
				}
				...
				setEyeState(eye, revealing, revealing ? "不再显示此工作区隐藏的会话" : "显示此工作区隐藏的会话");
			};
			/**
			...
			*/
			...
			function decorate() {
				...
				const hiddenWorkspaces = /* @__PURE__ */ new Set();
