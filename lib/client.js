window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function SettingsCard({ t, scope, describe, getLlm, getCredentials }) {
			...
			* （清掉是为了不把别处改进来的值一直盖住）。
			*/
			const [hiddenEnabledDraft, setHiddenEnabledDraft] = (0, react.useState)(void 0);
			(0, react.useEffect)(() => {
				const llm = getLlm();
				if (llm === void 0) {
					setDirectory({
			...
			(0, react.useEffect)(() => {
				if (hiddenEnabledDraft === void 0) return;
				if (hiddenEnabledValue === hiddenEnabledDraft) setHiddenEnabledDraft(void 0);
							...
							children: routes === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
								...
								children: [
									...
									provider !== "" && !providerKnown ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
										value: provider,
										children: t("notInList", { id: provider })
									}) : null
								]
			...
			*/
			const renderToggleRow = (options) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							...
							children: t(options.title)
						}), options.hint === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							className: "stp-hint",
							children: t(options.hint)
						})]
					}),
					options.overridden === true ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: "stp-overridden",
						children: t("overridden")
					}) : null,
			...
			*/
			const renderHiddenToggle = () => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "stp-field",
				children: renderToggleRow({
					...
					onChange: (next) => {
						...
						scope.set("hiddenEnabled", next).catch(() => {
							setHiddenEnabledDraft(void 0);
							setFailed(true);
						});
					}
				})
			...
			*/
				...
				return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "stp-label",
								children: t("rescueTitle")
							})
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							className: "stp-hint",
							children: t("rescueHint", { count: hiddenCount })
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
					...
					className: "stp-body",
					children: [
						visibleFields.map((desc) => desc.field === "provider" && modelDesc !== void 0 ? renderModelPair(desc, modelDesc) : renderField(desc)),
						renderHiddenToggle(),
...
`;
		...
		*/
		function installHiddenSessions(ctx, scope, getT) {
		...
		const zh = {
			...
			templateLabel: "标题格式",
			templateHint: "占位符：{YYYY} {MM} {DD} {HH} {mm} {ss} {type} {topic}。日期部件可任意拼接（如 {MMDD}、{YYYYMMDD}）；不写 {type} 就没有分类，不写 {topic} 就没有主题",
			...
			hiddenEnabledHint: "启用后，会话行悬停时行尾显示隐藏按钮，可将该会话从侧边栏隐藏；「工作区」标题行放大镜左侧的按钮用于整体显示或收起全部已隐藏的会话。隐藏仅影响侧边栏的呈现，不影响会话内容、搜索与标题生成，也不会删除任何数据。关闭后，上述按钮与隐藏效果均停止执行；已设置的隐藏列表将保留，重新启用时继续生效。",
			rescueTitle: "隐藏的会话",
		...
		const en = {
			...
			templateHint: "Placeholders: {YYYY} {MM} {DD} {HH} {mm} {ss} {type} {topic}. Date parts can be combined freely (e.g. {MMDD}, {YYYYMMDD}); omit {type} to drop the type, omit {topic} to drop the topic.",
			...
			credentialsUnknown: "Credential status is unavailable, so the list is derived from the settings document alone and may include providers that are not configured",
			...
			hiddenEnabledHint: "When enabled, hovering a session row reveals a hide button at the end of the row, and the button left of the search icon in the Workspaces header shows or collapses every hidden session. Hiding only affects the sidebar — it never changes session content, search, or title generation, and deletes no data. When disabled, those buttons and the hiding effect stop; the hidden list is kept and applies again once you re-enable it.",
			rescueTitle: "Hidden sessions",
			rescueHint: "{count} session(s) currently hidden. Hiding only affects the sidebar; it never changes session content or search.",
		...
		* 拨一下即纠正。
		*/
		const LOCK_STATE_CACHE = /* @__PURE__ */ new Map();
		/** 缓存上限：长开的页面里会话可能很多，别让它无界生长。 */
		const LOCK_STATE_CACHE_MAX = 200;
		function readCachedLockState(sessionId) {
			return LOCK_STATE_CACHE.get(sessionId);
		}
		function writeCachedLockState(sessionId, locked) {
		...
		function apply(ctx) {
			...
			*/
				...
				sub.slots.inject(SLOT, () => sub.slots.register({
					...
					inject: (sessionId) => ({
						...
						rename: (title) => {
							...
							runLine(sessionId, `${RENAME_LINE} ${title}`);
						},
						lock: () => {
							writeCachedLockState(sessionId, true);
							runLine(sessionId, LOCK_LINE);
						},
						unlock: () => {
							writeCachedLockState(sessionId, false);
							runLine(sessionId, UNLOCK_LINE);
						},
						readState: () => {
							const cached = readCachedLockState(sessionId);
							if (cached !== void 0) return Promise.resolve(cached ? "locked" : "unlocked");
							return runLine(sessionId, STATE_LINE).then((text) => {
								if (typeof text === "string") writeCachedLockState(sessionId, text.trim() === "locked");
								return text;
							});
						},
			...
			ctx.inject(["settingsScope"], (sub) => {
				installHiddenSessions(sub, sub.settingsScope.bind({ namespace: SETTINGS_NS }), getT);
			});
		}
		//#endregion
		exports.apply = apply;
