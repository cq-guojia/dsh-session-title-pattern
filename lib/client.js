...
`;
		...
		*/
		...
		function GenerateTitleAction({ useSession, generate, rename, lock }) {
		...
		*/
		...
		function apply(ctx) {
			...
			const runLine = (sessionId, line) => {
				...
				commands.execute(sessionId, line, []).catch(() => void 0);
			};
			ctx.inject(["slots"], (sub) => {
				sub.slots.inject(SLOT, () => sub.slots.register({
					...
					inject: (sessionId) => ({
						...
						rename: (title) => runLine(sessionId, `${RENAME_LINE} ${title}`),
