...
**1. 头部按钮 → 小面板** —— **标题右侧第一个**位置有个铅笔图标，**点开**是一个小面板，含三个动作：

- **根据对话重新生成** —— 模型对整段对话重新总结（会话正在生成回答时禁用）
- **锁定当前标题** —— 保持现在的字不动、停止自动更新；想恢复自动，再点一次「根据对话重新生成」即可
- **手动改标题** —— 输入新标题回车，写入后**自动锁定**

锁定与「手动改完自动锁定」用的是平台内置的标题固定机制（来源 = 用户），全链路生效：
侧边栏、自动调度、我们的重算计时器都认它。面板走 `/retitle`、`/title-lock`、`/title-rename`
三条命令，也可以在输入框里直接敲。
