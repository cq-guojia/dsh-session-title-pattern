...
![设置面板：模型开关、重算间隔、模型选择、超时、输出上限、标题格式、长度上限](docs/images/settings-card.png)
...
| **超时** / **输出标题最大Token** | 仅模型模式。后者单位是 token（128 大致相当于 200 个汉字），只是防止模型啰嗦的保险丝 |
| **标题格式** / **标题长度上限** | 两种模式都生效 |
...
| `maxOutputTokens` | number | `128` | 单次调用**输出** token 上限。128 大致相当于 200 个汉字，而标题只有一行，所以它是一根保险丝，一般不用改。**带推理（thinking）的模型会把思考也算进来**，因此不要调得太小 |
