...
| `hiddenEnabled` | boolean | `true` | 隐藏会话功能的总开关。关掉后左侧整套功能停止执行，**已设过的隐藏列表保留** |
| `hiddenSessions` | string[] | `[]` | 被隐藏的会话 id（见[隐藏会话](#隐藏会话)）。界面上由眼睛按钮写，不用手填 |
| `revealHiddenAll` | boolean | `false` | 是否把被隐藏的会话显示出来（「工作区」行那只眼睛） |
...
| 位置 | 作用 |
| --- | --- |
| **会话行**（悬停时行尾「…」左边浮现的眼睛） | 隐藏 / 取消隐藏**这一条** |
| **「工作区」区域标题行**（放大镜左边那只眼睛） | **总开关**：一键全显 / 全隐 |

鼠标停在任意一只眼睛上**立刻**弹出说明（原生 `title` 要等约一秒，所以这里是自己画的
提示气泡），不必猜它是干什么的。

> 曾做过「每个工作区各自一个开关，覆盖总开关」，实机用起来嫌碎（眼睛只在该工作区确有
> 隐藏会话时才出现，看到时说不清它管的是哪一层），已去掉，只留这一个总开关。

被显示出来的隐藏会话只有**一档视觉区分：颜色更淡（半透明）**，不加图标、不加文字、
不加删除线 —— 一眼能看出它和普通会话不同，又不会抢眼。

**当前正在打开的那一条会先留着、但同样淡着显示**：点隐藏后它仍在侧边栏（避免正在聊的
内容突然"消失"），颜色和「开关打开时的隐藏项」一模一样 —— 一眼能看出它已经被隐藏了；
切到别的会话后它自然按规则消失。

### 可以整体关掉

设置卡片里有一项「**启用隐藏会话**」（默认打开）。关掉它，侧边栏这一整套 —— 会话行上的
眼睛、放大镜左边那只眼睛、连同所有隐藏效果 —— **全部停止执行**，界面立刻回到一个没有
任何隐藏的状态，并且不再往设置里写任何东西。

**关掉 ≠ 重置**：你设过的隐藏列表与显示开关都原样保留，以后再打开，该隐藏的还是隐藏着。
要真正清空隐藏列表，用设置卡片里的「全部取消隐藏」。

两点交互约定：

- 这一项**一点即生效**（不走卡片上那个「保存」）—— 它是"要不要用这个功能"的总闸，
  改了就该立刻看到界面变化。紧跟在它下面的「隐藏的会话」那一块同样是点了立即生效。
- 关掉后，「隐藏的会话」那一块**整块不再显示** —— 功能都不执行了，谈"隐藏了多少条"、
  给一个"全部取消隐藏"按钮都没有意义。
...
- **依赖上游的 CSS 类名后缀**：`_sessionRow` / `_searchResultRow` / `_sectionHeader` /
  `_searchSlot` / `_rowActions`。哈希前缀每个模块都不同、且随构建变化，所以只按后缀匹配。
