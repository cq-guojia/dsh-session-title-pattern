...
| 模型（默认折叠） | provider、model（**都是下拉选择**，选项来自你在「设置 → 模型」里配好的供应商与模型）、超时、输出上限 |
| 标题格式（默认折叠） | 分隔符、标题长度上限 |

改动是**暂存**的：改完点「保存」才写入，点「放弃」丢弃。每个字段会标出是否**已覆盖**，
可以单字段「恢复默认」，也可以整体「全部恢复默认」——恢复默认就是删掉你写的那一层，
让它重新继承下面的 `cordis.patch.yml` 取值。
