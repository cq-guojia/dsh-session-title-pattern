...
- 渲染上复用了既有的 `renderToggle`，但那一支原先写死了 `'llm' / 'rules'`（服务
  `mode` 字段）。给 `FieldDesc` 加了可选的 `toggle: { on, off }`，`mode` 与
  `hiddenEnabled` 各自声明端点，开关行不再认识任何具体字段。布尔字段的草稿仍按
  文本走（`'true'` / `'false'`），与卡片「暂存 → 比对 → 保存」那一套保持一致。
- 这一项**进 `FIELDS`**（走正常的保存流程），刻意不同于「全部取消隐藏」那种即时生效的
  自救按钮：前者是一个可长期保持的偏好，理应能预览、能放弃、能标「未保存」。
